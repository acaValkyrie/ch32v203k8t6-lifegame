...
// CH32V003 Pin Definitions
    ...
    #define PIN_CS 2  // PC2
#endif
#define PIN_RESET PA3  // PC3
#define PIN_DC    PA4  // PC4
#define SPI_SCLK  PA5  // PC5
#define SPI_MOSI  PA7  // PC6

#define DATA_MODE()    (GPIOA->BSHR |= 1 << PIN_DC)  // DC High
#define COMMAND_MODE() (GPIOA->BCR |= 1 << PIN_DC)   // DC Low
#define RESET_HIGH()   (GPIOA->BSHR |= 1 << PIN_RESET)
#define RESET_LOW()    (GPIOA->BCR |= 1 << PIN_RESET)
#ifndef ST7789_NO_CS
    #define START_WRITE() (GPIOA->BCR |= 1 << PIN_CS)   // CS Low
    #define END_WRITE()   (GPIOA->BSHR |= 1 << PIN_CS)  // CS High
#else
    #define START_WRITE()
    #define END_WRITE()
#endif
...
#ifndef ST7789_NO_CS
    // PC2 - CS
    GPIOA->CFGLR &= ~(0xf << (PIN_CS << 2));
    GPIOA->CFGLR |= (GPIO_CNF_OUT_PP | GPIO_Speed_50MHz) << (PIN_CS << 2);
#endif

    // PC3 - RESET
    GPIOA->CFGLR &= ~(0xf << (PIN_RESET << 2));
    GPIOA->CFGLR |= (GPIO_CNF_OUT_PP | GPIO_Speed_50MHz) << (PIN_RESET << 2);

    // PC4 - DC
    GPIOA->CFGLR &= ~(0xf << (PIN_DC << 2));
    GPIOA->CFGLR |= (GPIO_CNF_OUT_PP | GPIO_Speed_50MHz) << (PIN_DC << 2);
